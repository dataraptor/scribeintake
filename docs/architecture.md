# Architecture — ScribeIntake

> The one thing to take away: **the safety guarantee comes from code, not from the model
> choosing to call a tool.** Every patient turn is run through a deterministic extractor → rule
...
([`core/.../orchestrator.py`](../core/src/scribeintake/orchestrator.py), spec §6). The
**EXTRACT** and **GATE** boxes contain **no LLM call**; everything downstream of the gate is the
agent.
...
\* **Model pins** (spec §19): the intake loop targets `claude-sonnet-4-6`, summary/triage/judge
target `claude-opus-4-8`. The wired demo deployment is **Azure GPT-5.5** (a single reasoning model
for both roles) — a documented provider deviation from Split 03; the seam is model-agnostic, so
pointing it back at Claude is a config change. See [PROGRESS](../tmp/split/PROGRESS.md) Split 03.

### The seven steps (spec §6)

1. **Extract** — `safety/extractor.py` turns the raw turn into a typed `Signals` object using
   regex + numeric parsing. No model, no network. Deterministic and unit-frozen.
2. **Gate** — `safety/rules.py` evaluates the declarative red-flag rule set (Appendix A) against
   those signals. Emits `CLEAR / CAUTION / CRISIS / EMERGENCY`.
...
4. **Agent loop** — otherwise the Sonnet/GPT loop runs with three tools (`record_intake`,
   `retrieve_guideline`, `assess_escalation`). `assess_escalation` is a **second, independent**
   net for oblique danger — it can raise the floor but can never lower the code gate.
5. **Completion check** — `intake/state_machine.py` (code) decides when the required canonical
   slots are filled; until then the agent asks the next adaptive follow-up.
6. **Summary** — `build_summary` is a terminal Opus call using **native structured outputs**, so
   the SOAP is schema-valid by construction (not validated-after-the-fact).
7. **Triage** — `suggest_triage` clamps the predicted band to **never fall below** the safety
   floor set in steps 2–4 (monotonic per session).

---

## In-process wiring (spec §6 / §14)
...
- `eval/` and `observability/` **import `core` in-process** — that is what makes eval runs
  isolated and parallel-safe (fresh SQLite per run; the orchestrator is **stateless per turn**:
  load state → run → save).
- The **browser** frontend can't import a Python module, so it is the one component that talks
  over **HTTP/SSE** — through `api/`, a *thin adapter* that only reshapes what the orchestrator
  already produced (the §6 "no safety/model logic in `api/`" grep is a passing test). This is the
  D1 deviation from the spec's Streamlit pin; the load-bearing "no HTTP where it matters
  (parallel-safe evals)" principle is preserved.

---

## Two-tier CI (spec §15 / §23)
...
flowchart TD
    ...
    G --> L["ruff check"]
    G --> T["pytest -m 'not live'  (555 tests)"]
...
- **Per-commit (free, deterministic):** lint + the deterministic test tier + the four gated eval
...
- **Nightly (key-gated, distributional):** the full ×N eval, the LLM judge, RAGAS retrieval evals,
  and the cost/observability report. **Reported, never asserted** — a one-point sampling wobble
  must not break the build; only the still-gated deterministic metrics are build-breaking.
...
2. The gate runs in **code, upstream of the LLM** — patient text can never disable it (the
